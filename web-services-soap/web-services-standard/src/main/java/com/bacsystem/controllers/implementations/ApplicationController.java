package com.bacsystem.controllers.implementations;

import com.bacsystem.controllers.contracts.IApplicationController;
...
public class ApplicationController implements IApplicationController {
    ...
    public ResponseEntity<Page<ApplicationDto>> pageExecute(
            ...
            String page, @RequestParam(name = "size", required = false, defaultValue = "5") String size) {
        log.info("page {}", page);
        log.info("size {}", size);
    ...
    public ResponseEntity<Integer> calculateExecute(
            @RequestParam(name = "a") String a,
            @RequestParam(name = "b") String b) {
        return new ResponseEntity<>(this.applicationService.calculate(Integer.parseInt(a), Integer.parseInt(b)), HttpStatus.OK);
    }
