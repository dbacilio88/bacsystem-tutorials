...
public class ApplicationResponse {
    ...
    public Mono<ServerResponse> getResponse(final ResponseBase response, final int code) {
        log.info("res {}", response);
        return build(response, code);
    }

    public Mono<ServerResponse> getResponse(final int code) {
        return build(new ResponseBase(), code);
    }

    private Mono<ServerResponse> build(final ResponseBase response, final int code) {
    ...
    public Mono<ResponseEntity<Object>> getResponseError(final ResponseBase response, final int code) {
        log.info("res {}", response);
        return buildError(response, code);
    }

    private Mono<ResponseEntity<Object>> buildError(final ResponseBase response, final int code) {
        response.setCode(code);
        response.setDateTime(new Date());
        response.setMessage(response.getMessage());
